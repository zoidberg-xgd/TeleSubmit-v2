...
本文档详细介绍所有管理脚本的功能和使用方法。

## 📋 脚本概览

| 脚本 | 用途 | 适用场景 |
|------|------|----------|
| `quickstart.sh` | 快速启动向导 | 首次部署，不确定用哪种方式 |
| `install.sh` | 一键安装 | 首次部署，需要完整向导 |
| `deploy.sh` | Docker 部署 | 生产环境，容器化部署 |
| `start.sh` | 启动机器人 | 已配置环境，直接启动 |
| `restart.sh` | 重启机器人 | 修改配置后重启 |
| `update.sh` | 更新代码 | 拉取最新代码 |
| `upgrade.sh` | 功能升级 | 版本升级，数据迁移 |
| `uninstall.sh` | 卸载 | 完全移除或备份卸载 |
